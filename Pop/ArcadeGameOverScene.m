...
@implementation ArcadeGameOverScene
...
//        
        ...
        GKScore *scoreReporter = [[GKScore alloc] initWithLeaderboardIdentifier:@"ArcadeLB"]; scoreReporter.value = theScore; [scoreReporter reportScoreWithCompletionHandler:^(NSError *error) { if (error != nil){ NSLog(@"Submitting score failed"); } else { NSLog(@"Submitting score succeeded"); } }];
        
        NSInteger lastScore = [[NSUserDefaults standardUserDefaults] integerForKey:@"arcLastScore"];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([[touchedNode name] isEqualToString:@"restart"]||[[touchedNode name] isEqualToString:@"restar"]) {
        ...
        if([deviceType hasPrefix:@"iPad"]){
            load.image = [UIImage imageNamed:@"pop_arcade_tutorial.png"];
